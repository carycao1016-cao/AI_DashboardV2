...
### 12.1 Boundary-recognition artifacts

The ingestion pipeline uses three internal, versioned artifacts before writing final extraction objects. They support generic Tab Books and are never exposed through the Client API:
...
- `WorkbookScanSummary` is generated deterministically by Python. It records Sheet dimensions, used ranges, merged and hidden regions, row-density and value-type signatures, A-column and first-non-empty-cell clues, candidate question/Base rows, and bounded contextual samples. It is an AI input summary, not a replacement for raw workbook evidence.
- `TableBoundaryProposal` is structured AI output. It proposes physical table ranges and regions such as title, header, Base, data, footnote, separator and possible significance locations. It may include confidence and evidence references, but is never accepted as final source data.
- `BoundaryValidationResult` is generated by Python after re-reading the original workbook at the proposed coordinates. It validates Sheet bounds, region overlap, numeric density, Header construction, Base evidence, and adjacent-table conflicts before final extraction objects are written.

The summary must be bounded or chunked for large Sheets. A-column content is a high-priority clue, but neither question numbers nor table boundaries may be assumed to exist in column A.
