  ...
  const filteredSheets = useMemo(
    () => {
      const recognizedSheets = recognitionResult?.result.sheets?.map((sheet) => {
        const hasReview = sheet.boundary_validations.some((validation) => validation.outcome === "review_required" || validation.outcome === "rejected");
        return { name: sheet.sheet_name, family: "AI 识别", tables: sheet.boundary_proposals.length, status: (hasReview ? "需 Review" : "已验证") as Status, range: sheet.boundary_proposals[0]?.source_range || "未返回范围" };
      });
      const inventory = recognizedSheets?.length ? recognizedSheets : sheets;
      return inventory.filter((sheet) => `${sheet.name} ${sheet.family}`.toLowerCase().includes(query.toLowerCase()));
    },
