...
export function App() {
  ...
  const [uploadWave, setUploadWave] = useState("Wave 1");
  ...
  return (
    <div className={`app-shell ${assistantOpen ? "assistant-visible" : ""}`}>
      ...
      {showUpload && <div className="modal-backdrop" role="presentation"><div className="upload-modal" role="dialog" aria-modal="true" aria-labelledby="upload-title"><div className="modal-topline" /><div className="modal-heading"><div><span className="section-kicker">SOURCE FILE VERSION</span><h2 id="upload-title">上传新版本</h2><p>市场和 Wave 属于文件版本，不是项目固定属性。</p></div><button className="icon-button" onClick={() => setShowUpload(false)} title="关闭"><X size={18} /></button></div><div className="drop-zone"><Upload size={24} /><strong>拖入 Excel 或 CSV 文件</strong><span>本地演示模式：文件不会上传到服务器</span><button className="button secondary"><FolderOpen size={15} />选择文件</button></div><div className="form-grid upload-context-grid"><label>文件市场范围<select value={uploadMarket} onChange={(event) => setUploadMarket(event.target.value)}><option>Global（总体）</option><option>多市场</option><option>US</option><option>CN</option><option>UK</option></select></label><label>数据波次<select value={uploadWave} onChange={(event) => setUploadWave(event.target.value)}><option>Wave 1</option><option>Wave 2</option><option>Tracking</option></select></label></div><div className="modal-foot"><span><ShieldCheck size={14} />服务端将负责 MIME、扩展名和工作簿结构校验</span><button className="button primary" onClick={() => { setMarketScope(uploadMarket); setWaveScope(uploadWave); setShowUpload(false); }}>完成</button></div></div></div>}
      {showProjectCreate && <div className="modal-backdrop" role="presentation"><div className="project-modal" role="dialog" aria-modal="true" aria-labelledby="project-title"><div className="modal-topline" /><div className="modal-heading"><div><span className="section-kicker">PROJECT SETUP</span><h2 id="project-title">新建项目</h2><p>项目只保存研究上下文。市场范围和 Wave 会在上传文件版本时指定。</p></div><button className="icon-button" onClick={() => setShowProjectCreate(false)} title="关闭"><X size={18} /></button></div><div className="form-grid"><label>项目名称<input value={draftProjectName} onChange={(event) => setDraftProjectName(event.target.value)} placeholder="例如：Brand Tracker" autoFocus /></label></div><div className="modal-foot"><span><ShieldCheck size={14} />创建后可上传多个市场和多个 Wave 的文件版本</span><div className="modal-actions"><button className="button secondary" onClick={() => setShowProjectCreate(false)}>取消</button><button className="button primary" onClick={createProject} disabled={!draftProjectName.trim()}><Plus size={15} />创建项目</button></div></div></div></div>}
    </div>
  );
}
